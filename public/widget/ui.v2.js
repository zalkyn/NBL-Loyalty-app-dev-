/* =============================================================================
   ...
   notify:reward:close
   notify:info:open       { payload: { text, claim?, data? } }
   notify:info:close
...
(function () {
    ...
    function init() {
        ...
        var WIDGET_CONFIG = {
            // ── Widget behaviour ─────────────────────────────────────────────────
            // Show the "Request Discount Code" button while auto-submitting?
            showSubmitButtonOnAuto: false,

            // Show the Active Rewards / Recent Activity sections on the Home tab?
            showHomeRewardsSection: true,
            showHomeActivitiesSection: true,

            // Items per page in the home section lists.
            ...
            labels: {
                ...
                emptyRewards: lbl('emptyRewards'),
                emptyActivity: lbl('emptyActivity'),
        ...
        (function applyWidgetConfigOverrides() {
            ...
            if (wc.homeActivitiesPerPage !== undefined) WIDGET_CONFIG.homeActivitiesPerPage = Math.max(1, Number(wc.homeActivitiesPerPage) || 7);
        ...
        function referralTabHTML(referralLink) {
            ...
            var isSubscription = trigger === 'subscription';
            var step2Title = isSubscription ? 'Friend subscribes' : 'Friend makes a purchase';
            var step2Desc = isSubscription
                ? 'They click your link and start a subscription'
                : 'They click your link and place their first order';
        ...
        function referralModalHTML() {
            ...
            if (trigger === 'subscription') {
                var rewardParts = [];
                if (referrer.firstOrderPoints > 0)
                    rewardParts.push('Earn <strong>' + formatNumber(referrer.firstOrderPoints) + ' points</strong> when your friend places their first subscription order.');
            ...
            } else if (referrer.firstOrderPoints > 0) {
                referrerRewardText = 'Earn <strong>' + formatNumber(referrer.firstOrderPoints) + ' points</strong> when your friend completes their first order.';
            }
            ...
            if (referred.discountValue) {
                ...
                var friendOrderNote = referred.minimumOrderValue
                    ...
                    : (trigger === 'subscription' ? ' for your first subscription order' : ' for your first order');
                friendRewardRows.push('🎁 You get <strong>' + voucherValue + '</strong>' + friendOrderNote + '.');
            }
            if (trigger === 'subscription' && referred.allowRenewalReward && referred.renewalPoints > 0) {
        ...
        loyaltyApp.tab.renderEarnPointsList = function () {
            ...
            function buildLabel(rule) {
                ...
                var earning = ruleConditions.earning || {};
                var referralConditions = ruleConditions.referral || {};
                var review = ruleConditions.review || {};

                if (type === 'REVIEW') {
                    var parts = [];
                    if (review.text) parts.push(formatPoints(review.text) + ' for text review');
                    if (review.image) parts.push(formatPoints(review.image) + ' for photo review');
                    if (review.video) parts.push(formatPoints(review.video) + ' for video review');
                    return parts.join('. ') || 'Earn points for reviews';
                }

                if (type === 'ORDER') {
                    if (earning.type === 'incremental' && earning.rate)
                        return 'Get ' + formatPoints(earning.rate.points) + ' for every ' + currencySymbol + formatNumber(earning.rate.amount) + ' spent';
                ...
                if (type === 'REFERRAL') {
                    ...
                    if (trigger === 'subscription') {
                        if (referrer.firstOrderPoints > 0)
                            p2.push('Earn ' + formatPoints(referrer.firstOrderPoints) + ' when your friend places their first subscription order');
                        if (referrer.allowRenewalReward && referrer.renewalPoints > 0)
                            p2.push('Earn ' + formatPoints(referrer.renewalPoints) + ' for each renewal');
                    } else {
                        if (referrer.firstOrderPoints > 0)
                            p2.push('Earn ' + formatPoints(referrer.firstOrderPoints) + ' when your friend makes their first order');
                    }
                    if (referred.discountValue)
                        p2.push('Your friend gets ' + formatDiscount(referred.discountValue, referred.discountType, currencySymbol) + ' on their first order');
                    return p2.length ? p2.join('. ') : 'Earn points by referring friends';
                }

                return 'Earn ' + formatPoints(earning.fixedPoints || rule.pointsCost || 0) + ' for completing this action';
            }
            ...
            function buildPointsText(rule) {
                ...
                if (type === 'REVIEW') {
                    ...
                    if (review.text) reviewPointParts.push(formatPoints(review.text) + ' text');
                    if (review.image) reviewPointParts.push(formatPoints(review.image) + ' photo');
                ...
                if (type === 'REFERRAL') {
                    ...
                    if (trigger === 'subscription') {
                        var pointParts = [];
                        if (referrer.firstOrderPoints > 0)
                            pointParts.push(formatPoints(referrer.firstOrderPoints) + ' (first order)');
                        if (referrer.allowRenewalReward && referrer.renewalPoints > 0)
                            pointParts.push(formatPoints(referrer.renewalPoints) + ' (renewals)');
                        return pointParts.length ? pointParts.join(' + ') : '—';
                    }
                    if (referrer.firstOrderPoints > 0)
                        return formatPoints(referrer.firstOrderPoints) + ' (one-time)';
                    return '—';
                }

                if (type === 'ORDER') {
                    if (earning.type === 'incremental' && earning.rate)
                        return formatPoints(earning.rate.points) + ' per ' + currencySymbol + formatNumber(earning.rate.amount);
                    if (earning.type === 'fixed')
                        return formatPoints(earning.fixedPoints) + ' per order';
                }

                return formatPoints(earning.fixedPoints || rule.pointsCost || 0);
            }
            ...
            if (container && refRule) {
                ...
                if (isSubscription) {
                    if (referrer.firstOrderPoints > 0)
                        rewardRows.push(rewardRow('You', 'nbl-referral-reward-you-v1',
                            formatNumber(referrer.firstOrderPoints) + ' points',
                ...
                if (referred.discountValue) {
                    ...
                    var orderNote = referred.minimumOrderValue
                        ? 'On orders over ' + currencySymbol + formatNumber(referred.minimumOrderValue)
                        : (isSubscription ? 'On first subscription order' : 'On first order');
                    rewardRows.push(rewardRow('Friend', 'nbl-referral-reward-friend-v1', voucherLabel, orderNote));
                    if (referred.allowRenewalReward && referred.renewalPoints > 0)
        ...
        eventBus.on('widget:first-open', function () {
            ...
            infoEl.querySelector('.nbl-notify-info-close-v1').addEventListener('click', closeInfo);
        ...
        eventBus.on('notify:info:claim:start', function (data) {
            var rule = data && data.data && data.data.rewardRule;
            if (!rule || !rule.id) return;
            loyaltyApp.requestToGetRewardVoucher({ rewardRuleId: rule.id, title: data.data.title });
        });
