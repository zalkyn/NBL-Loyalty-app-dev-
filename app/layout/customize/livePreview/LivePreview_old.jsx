...
import { CSS_DEFAULTS, LABEL_DEFAULTS, DS } from "../constants/cssVarsConfig";
import { Icon, LauncherIcon } from "../components/Icon";
...
const LivePreviewPanel = memo(function LivePreviewPanel({
    ...
    const launcherIconName = get("--nbl-launcher-icon", "'gift'").replace(/^'|'$/g, "");
    ...
    const widgetPopup = widgetOpen && (
        <div data-nbl-preview style={{
            ...
            <div style={{ background: headerBg, padding: "18px 16px 16px", position: "relative" }}>
                ...
                }}><Icon name="x" size={13} strokeWidth={2.5} /></div>
            ...
            <div style={{ background: navBg, borderBottom: `1.5px solid ${navBorderColor}`, display: "flex", alignItems: "stretch", padding: "0 10px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", padding: "0 4px" }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: navBg, border: `1.5px solid ${navBorderColor}`, display: "flex", alignItems: "center", justifyContent: "center", color: navItemColor }}><Icon name="chevronRight" size={13} strokeWidth={2.5} /></div>
                </div>
            </div>
            ...
            <div style={{ background: surface, padding: "12px 10px", position: "relative", overflowY: "auto", flex: 1 }}>
                ...
                {resolvedTabIndex === 0 && !isOverlay && (
                    <>
                        {[
                            { icon: "rewards", label: lbl("homeCardBrowse") },
                            { icon: "lightning", label: lbl("homeCardEarn") },
                            { icon: "referral", label: lbl("homeCardRefer") },
                        ].map((item) => (
                            <div key={item.label} style={{ background: headerBg, borderRadius: cardRadius, padding: "12px 14px", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                    <Icon name={item.icon} size={16} color={homeNavColor} />
                                    <span style={{ fontSize: 13, fontWeight: 600, color: homeNavColor }}>{item.label}</span>
                                </div>
                                <Icon name="chevronRight" size={15} color={homeNavColor} />
                            </div>
                        ))}
                        <div style={{ height: 8 }} />
                        {/* Active Rewards */}
                        <div style={{ border: `1.5px solid ${border}`, borderRadius: cardRadius, overflow: "hidden", marginBottom: 8 }}>
                            <div style={{ background: hscHeaderBg, padding: "9px 12px", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", gap: 7 }}>
                                <Icon name="rewards" size={14} color={textColor} />
                            ...
                            {[{ label: "Voucher $5" }, { label: "Voucher $10" }].map((v, i) => (
                                <div key={i} style={{ background: surface, padding: "10px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: i === 0 ? `1px solid ${border}` : "none" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                        <Icon name="rewards" size={16} color={navActiveColor} />
                                        <span style={{ fontSize: 13, fontWeight: 600, color: textColor }}>{v.label}</span>
                                    </div>
                                    <Icon name="chevronRight" size={14} color={navActiveColor} />
                                </div>
                            ))}
                        </div>
                        {/* Recent Activity */}
                        <div style={{ border: `1.5px solid ${border}`, borderRadius: cardRadius, overflow: "hidden" }}>
                            <div style={{ background: hscHeaderBg, padding: "9px 12px", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", gap: 7 }}>
                                <Icon name="lightning" size={14} color={textColor} />
                ...
                {resolvedTabIndex === 1 && !isOverlay && (
                    <div>
                        {[
                            { icon: "referral", label: "Refer a Friend", sub: "100 points (first order)" },
                            { icon: "cart", label: "Direct Purchase", sub: "50 points per $1" },
                            { icon: "star", label: "Loox Review Written", sub: "10 points text · 20 points photo" },
                        ].map((item, i) => (
                            <div key={i} style={{ background: rewardItemBg, border: `1.5px solid ${rewardItemBorder}`, borderRadius: cardRadius, padding: "12px 13px", marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
                                <Icon name={item.icon} size={20} color={navActiveColor} />
                                <div style={{ flex: 1 }}>
                ...
                {resolvedTabIndex === 2 && !isOverlay && (
                    <div>
                        {[
                            ...
                            <div key={i} style={{ background: rewardItemBg, border: `1.5px solid ${rewardItemBorder}`, borderRadius: cardRadius, padding: "12px 13px", marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
                                <Icon name="gift" size={22} color={navActiveColor} />
                                <div style={{ flex: 1 }}>
                                    <div style={{ fontSize: 13, fontWeight: 700, color: textColor }}>{item.label}</div>
                ...
                {previewScene === "notification-reward" && isOverlay && (
                    <>
                        <div style={{ opacity: 0.28, pointerEvents: "none" }}>
                            {[{ icon: "rewards", label: lbl("homeCardBrowse") }, { icon: "lightning", label: lbl("homeCardEarn") }].map((item) => (
                                <div key={item.label} style={{ background: headerBg, borderRadius: cardRadius, padding: "10px 12px", marginBottom: 7, display: "flex", alignItems: "center", gap: 10 }}>
                                    <Icon name={item.icon} size={15} color={homeNavColor} /><span style={{ fontSize: 12, color: homeNavColor }}>{item.label}</span>
                                </div>
                            ))}
                        </div>
                        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: `linear-gradient(135deg, ${notifyBgFrom} 0%, ${notifyBgTo} 100%)`, borderRadius: `0 0 ${radius} ${radius}`, padding: "16px 14px 20px" }}>
                            <div style={{ position: "absolute", top: 10, right: 10, width: 24, height: 24, borderRadius: "50%", background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", color: notifyColor }}><Icon name="x" size={12} strokeWidth={2.5} /></div>
                ...
                {previewScene === "notification-info" && isOverlay && (
                    <>
                        <div style={{ opacity: 0.28, pointerEvents: "none" }}>
                            {[{ icon: "referral", label: "Refer a Friend", sub: "100 pts" }, { icon: "cart", label: "Direct Purchase", sub: "50 pts/$1" }].map((item, i) => (
                                <div key={i} style={{ background: rewardItemBg, border: `1px solid ${border}`, borderRadius: cardRadius, padding: "10px 12px", marginBottom: 7, display: "flex", alignItems: "center", gap: 10 }}>
                                    <Icon name={item.icon} size={18} color={textMuted} />
                                    <div>
                        ...
                        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: `linear-gradient(135deg, ${notifyBgFrom} 0%, ${notifyBgTo} 100%)`, borderRadius: `0 0 ${radius} ${radius}`, padding: "16px 14px 20px" }}>
                            <div style={{ position: "absolute", top: 10, right: 10, width: 24, height: 24, borderRadius: "50%", background: "rgba(255,255,255,0.12)", display: "flex", alignItems: "center", justifyContent: "center", color: notifyColor }}><Icon name="x" size={12} strokeWidth={2.5} /></div>
                    ...
                    <>
                        <div style={{ opacity: 0.28, pointerEvents: "none" }}>
                            {[{ label: "Voucher $5", pts: "10 points" }, { label: "Voucher $30", pts: "300 points" }].map((item, i) => (
                                <div key={i} style={{ background: rewardItemBg, border: `1px solid ${border}`, borderRadius: cardRadius, padding: "10px 12px", marginBottom: 7, display: "flex", alignItems: "center", gap: 10 }}>
                                    <Icon name="gift" size={20} color={navActiveColor} />
                                    <div>
                                        <div style={{ fontSize: 13, fontWeight: 700, color: textColor }}>{item.label}</div>
                        ...
                        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "#0f172a", borderRadius: `0 0 ${radius} ${radius}`, padding: "16px 14px 20px" }}>
                            <div style={{ position: "absolute", top: 10, right: 10, width: 24, height: 24, borderRadius: "50%", background: "rgba(255,255,255,0.12)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}><Icon name="x" size={12} strokeWidth={2.5} /></div>
    ...
    const launcherButton = (
        <div onClick={() => setWidgetOpen((o) => !o)} style={{
            ...
            <div style={{ width: 38, height: 38, borderRadius: "50%", background: "rgba(0,0,0,0.18)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><LauncherIcon name={launcherIconName} size={20} color={launcherColor} /></div>
            <div>
    ...
    const modalPreview = (
        <div data-nbl-preview style={{
            ...
            <div style={{ position: "absolute", top: 16, right: 18, width: 28, height: 28, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: modalMutedColor, cursor: "pointer" }}><Icon name="x" size={16} strokeWidth={2} /></div>
            ...
            <div style={{ fontSize: 20, fontWeight: 800, color: modalTitleColor, marginBottom: 6, lineHeight: 1.25, display: "flex", alignItems: "center", gap: 8 }}>Get Your Referral Discount <Icon name="gift" size={20} color={modalTitleColor} /></div>
