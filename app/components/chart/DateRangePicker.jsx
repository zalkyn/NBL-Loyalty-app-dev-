 ...
 * Date range selector and optional interval selector.
 *
 * Exports:
 *   IntervalSelect   — bucket count dropdown (optional, hides when not useful)
 *   DateRangePicker  — preset select + Polaris calendar for custom ranges
 */
...
/**
 * Optional x-axis bucket count selector.
 *
 * When the user picks an interval, useChartSeries merges the raw buckets
 * down to that count — the chart shows exactly those data points.
 *
 * Hidden automatically when labelCount <= 4 (no value in reducing further).
 *
 * @param {Object}      props
 * @param {number}      props.labelCount  - Raw bucket count (pre-merge)
 * @param {number|null} props.interval    - Active selection; null = Auto
 * @param {Function}    props.onChange    - Change handler (e.target.value)
 *
 * @example
 * <IntervalSelect labelCount={labelCount} interval={interval} onChange={handleIntervalChange} />
 */
 ...
 *
 * Preset mode: dropdown immediately applies the range.
 * Custom mode: Polaris calendar with pending state — range applies only on "Apply".
 *
 * @param {Object}      props
 * @param {string}      props.preset
 * @param {Function}    props.onPresetChange
 * @param {string}      props.customStart        - Applied YYYY-MM-DD
 * @param {string}      props.customEnd          - Applied YYYY-MM-DD
 * @param {Function}    props.onCustomApply      - ({ start, end }) => void
 * @param {string}      props.granularity        - For the granularity badge
 * @param {number}      props.labelCount         - Passed to IntervalSelect
 * @param {number|null} props.interval
    ...
    return (
        <s-section>
            <s-stack direction="block" gap="base">
                ...
                {/* Date range + interval selects in a responsive 2-column grid */}
                <s-query-container>
                    <s-grid
                ...
                {/* Polaris calendar — custom mode only, hidden after Apply */}
